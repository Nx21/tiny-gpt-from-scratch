...
def build_stoi(vocab):
    ...
    return {i:j for i,j in zip(vocab, range(len(vocab)))}
...
def build_itos(vocab):
    ...
    return {j:i for i,j in zip(vocab, range(len(vocab)))}
...
def encode_string(text, stoi):
    ...
    return list(map(lambda c: stoi[c], text))
...
def decode_ids(ids, itos):
    ...
    return ''.join(map(lambda x: itos[x], ids))
...
def get_array_shape(arr):
    ...
    return make_1d_array(arr).shape
...
def get_array_dtype(arr):
    ...
    return make_1d_array(arr).dtype
...
def make_2d_random(rows, cols, seed):
    ...
    matrix = rng.random(size=(rows, cols), dtype=np.float64)
    return matrix
...
def naive_softmax_1d(logits):
    ...
    exp_arr = array_exp(logits)
    dem = sum_all(exp_arr)
    return np.array([i / dem for i in exp_arr])
...
def stable_softmax_1d(logits):
    ...
    arr = array_exp([x - c for x in logits])
    dim = sum_all(arr)
...
def stack_x_batch(data, offsets, block_size):
    ...
    return np.array([slice_x_at_offset(data, i, block_size) for i in offsets])
...
def stack_y_batch(data, offsets, block_size):
    ...
    return np.array([slice_y_at_offset(data, i, block_size) for i in offsets])
...
def allocate_count_matrix(vocab_size):
    ...
    return np.zeros(shape= (vocab_size, vocab_size), dtype=int)
